        ...
        columns: [
            ...
            { data: 'applicationUser.email', "width": "20%" },
            {
                data: 'orderStatus', "width": "15%", render : function (data)
                {
                    ...
                    return '<span style="display:inline-flex;align-items:center;font-size:12px;font-weight:500;padding:3px 10px;border-radius:4px;background:'
                        + bg + ';color:' + fg + '">' + data + '</span>';
                }
            },
                { data: 'orderTotal', "width": "15%", render: function (data) { return "$" + data.toFixed(2) } },
            {
                data: 'id', "width": "10%", render: function (data)
                {
                    return `<div class="d-flex gap-2 justify-content-end">
                                <a href="/admin/order/details?orderId=${data}" class="btn btn-sm btn-outline-info">
